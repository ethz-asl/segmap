...
def placeholder_inputs(batch_size, num_point, n_classes):
    pointclouds_pl = tf.placeholder(tf.float32, shape=(batch_size, num_point, 3))
    #labels_pl = tf.placeholder(tf.int32, shape=(batch_size))
    labels_pl = tf.placeholder(tf.int32, shape=(batch_size, n_classes))
...
def get_model(point_cloud, is_training, n_classes, bn_decay=None):
    ...
    net = tf_util.dropout(net, keep_prob=1.0, is_training=is_training, scope='dp3')
