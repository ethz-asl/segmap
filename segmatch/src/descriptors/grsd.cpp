...
void GrsdDescriptor::describe(const Segment& segment, Features* features) {
  ...
  // std::raise(SIGINT);
  // Grsd.computePointSPFHSignature(*cloud, *cloud_normals, (cloud->size())-1, 0, indices, hist_f1, hist_f2, hist_f3);
  ...
  // Return descriptor.
  // Eigen::VectorXf grsd_vec(10);
  // Grsd_vec = hist_tot.row(0);
  ...
  Feature grsd_feature("grsd");
  for (size_t j = 0u; j < 21; ++j){

      grsd_feature.push_back(
      FeatureValue("grsd_" + std::to_string(j), double(hist[j])));
      // std::cout<<double(Grsd_vec[j])<<std::endl;
  }

  features->replaceByName(grsd_feature);

  // Viz histogram.
  // pcl::GrsdSignature33 testss;
  // for (int ss=0; ss<33; ss++)
  // {
  //   testss.histogram[ss]=Grsd_vec[ss];
  // }
  // pcl::PointCloud<pcl::GrsdSignature33>::Ptr descriptors(new pcl::PointCloud<pcl::GrsdSignature33>());
  ...
  std::cout<<"Took: "<<secondsPassed<<" seconds!"<<std::endl;
}
